...
import com.kiwi.monthlymoneymanager.models.Budgets;
...
public class OperationsBean implements Serializable 
{
    ...
    {
        Query query = em.createNamedQuery("Budgets.findByUserId");
        List<Budgets> budgets = query.setParameter("userId", userId).getResultList();
        
            for (Budgets budget : budgets) 
            {
            if (budget.getSelectedCategory().getName().equals(selectedCategory)) 
            {
                budget.setSelectedAmount(expenseAmount);
                em.merge(budget);
            }
            //em.flush();
        }
